...
If `true`, the plugin exports the meta descriptions of all pages in your MkDocs site to a CSV file `site_dir/meta_descriptions.csv`. This is useful to review and keep track of all the meta descriptions in your pages, especially if you're maintaining a big site.

This feature uses the generated `sitemap.xml` to find all generated HTML pages and read the meta descriptions from those files, so it will also include pages that may be generated by other plugins, for example.
